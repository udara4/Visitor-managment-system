...
$name_for_project="VMS_Month_Record(".$dat.")";

	header('Content-Type: application/xls');
	header("content-disposition: attachment;filename=".$name_for_project.".xls");	
$sqlQuery = "SELECT Name,Ic,Gender,Phone,Department,Person_Meet,In_Time,Out_Time,Date,Address FROM inquery where MONTH(Date) = Month(CURRENT_DATE()) ORDER BY id ASC";
$resultSet = mysqli_query($db, $sqlQuery) or die("database error:". mysqli_error($db));
$inquerydata = array();
while( $inquery = mysqli_fetch_assoc($resultSet) ) {
	$inquerydata[] = $inquery;
